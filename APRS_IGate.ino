...
// Offset voor de configuratie in EEPROM
#define OFFSET_EEPROM      (0x0)

// Grootte van de EEPROM
#define EEPROM_SIZE        (184)

// Buffergrootte voor data
#define BUFFERSIZE         (260)

// GPIO-pinnen voor de modem RX en TX
#define MODEM_RX           (22)
#define MODEM_TX           (23)

// GPIO-pinnen voor de OLED display
#define OLED_SCL           (15)  // GPIO15
#define OLED_SDA           (4)   // GPIO4
#define OLED_RST           (16)

// GPIO-pin voor de TX LED
#define TX_LED             (27)

// Adres van de OLED display
#define OLED_ADR           (0x3C)  // Default 0x3C voor 0.9", voor 1.3" is het 0x78

// Timeout in seconden om de watchdog te activeren
...
void setup() {
  ...
  if (EEPROM.read(OFFSET_EEPROM) != storage.chkDigit) {
...
void setDra(byte rxFreq, byte txFreq, byte rxTone, byte txTone) {
  char buff[50];
  int txPart, rxPart;
  if (txFreq > 79) txPart = txFreq - 80;
  else txPart = txFreq;
  if (rxFreq > 79) rxPart = rxFreq - 80;
  else rxPart = rxFreq;
...
void saveConfig() {
  for (unsigned int t = 0; t < sizeof(storage); t++)
    EEPROM.write(OFFSET_EEPROM + t, *((char *)&storage + t));
  EEPROM.commit();
}

void loadConfig() {
  if (EEPROM.read(OFFSET_EEPROM + 0) == storage.chkDigit)
    for (unsigned int t = 0; t < sizeof(storage); t++)
      *((char *)&storage + t) = EEPROM.read(OFFSET_EEPROM + t);
}

void printConfig() {
  if (EEPROM.read(OFFSET_EEPROM + 0) == storage.chkDigit)
    for (unsigned int t = 0; t < sizeof(storage); t++)
      Serial.write(EEPROM.read(OFFSET_EEPROM + t));
